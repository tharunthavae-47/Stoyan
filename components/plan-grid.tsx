...
type PaidPlan = "basic" | "professional" | "business" | "premium"
type CheckoutRole = "employer" | "employee"
...
function getPaidPlanConfig(planId: string, plans: Plan[]): { plan: PaidPlan; role: CheckoutRole } | null {
  if (planId === "basic" || planId === "professional" || planId === "business") {
    return { plan: planId, role: "employer" }
  }
  if (planId === "premium" && plans.some((plan) => plan.id === "premium")) {
    return { plan: "premium", role: "employee" }
  }
  return null
}
...
export function PlanGrid({ plans }: { plans: Plan[] }) {
  ...
  const hasPaidPlans = plans.some((plan) => getPaidPlanConfig(plan.id, plans) !== null)

  async function startCheckout(planId: string) {
    const config = getPaidPlanConfig(planId, plans)
    if (!config) return
    ...
    <div>
      ...
      <div className="grid gap-0 border-t border-[#cdd6e0] lg:grid-cols-3">
        {plans.map((plan, index) => {
          ...
          const paidConfig = getPaidPlanConfig(plan.id, plans)
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                {plan.badge && (
                  <span className={"px-2.5 py-1 text-[0.68rem] font-semibold uppercase tracking-[0.08em] " + (highlighted ? "bg-[#2356d8] text-white" : "border border-[#c9d2dd] text-[#52667e]")}>
                    {plan.badge}
                  </span>
                )}
              </div>
              ...
              <div className="mt-6 flex items-end gap-2 border-b border-[#dfe4ea] pb-6">
                <span className="text-[2.75rem] font-medium leading-none tracking-[-0.05em] text-[#14243a]">{formatPrice(displayPrice)}</span>
                {displayPeriod && <span className="pb-1.5 text-[0.875rem] text-[#687384]">{displayPeriod}</span>}
              </div>
              {displayNote && <p className="mt-3 text-[0.72rem] font-semibold uppercase tracking-[0.06em] text-[#687384]">{displayNote}</p>}
              ...
              {paidConfig ? (
                <button
                  ...
                  className={"mt-6 inline-flex min-h-[50px] items-center justify-center gap-2 rounded-[4px] px-6 text-[0.9375rem] font-medium transition disabled:cursor-not-allowed disabled:opacity-60 " + (highlighted ? "bg-[#2356d8] text-white hover:bg-[#1844b6]" : "border border-[#d3dbe4] text-[#14243a] hover:bg-[#f2f5f8]")}
                >
                ...
                <Link
                  href={plan.ctaHref}
                  className={"mt-6 inline-flex min-h-[50px] items-center justify-center gap-2 rounded-[4px] px-6 text-[0.9375rem] font-medium transition " + (highlighted ? "bg-[#2356d8] text-white hover:bg-[#1844b6]" : "border border-[#d3dbe4] text-[#14243a] hover:bg-[#f2f5f8]")}
                >
                  {plan.ctaLabel}
              ...
              <ul className="mt-8 grid gap-3.5 border-t border-[#dfe4ea] pt-6">
                {plan.features.map((feature, i) => {
                  ...
                  return <li key={i} className="flex items-start gap-3 text-[0.875rem]">{included ? <Check className="mt-0.5 h-[17px] w-[17px] shrink-0 text-[#2356d8]" strokeWidth={2} /> : <Minus className="mt-0.5 h-[17px] w-[17px] shrink-0 text-[#a7b3c2]" strokeWidth={2} />}<span className={included ? "text-[#38465a]" : "text-[#98a4b3] line-through"}>{feature.label}</span></li>
                })}
              </ul>
            </div>
          )
      ...
      {error && <p className="mt-6 border-l-2 border-[#da3839] bg-[#fef2f2] px-4 py-3 text-[0.875rem] text-[#b91c1c]">{error}</p>}
    </div>
  )
}
