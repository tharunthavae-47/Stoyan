...
export function EmployerAIAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([{ role: "assistant", content: "Hallo! Ich bin dein persönlicher jobmatch24 KI-Assistent. Ich kann innerhalb von JobMatch24 nach passenden Kandidaten suchen, Profile vergleichen und dir bei Bewerbungen helfen. Ich merke mir die letzten 10 Nachrichten von dir, damit Folgefragen im Zusammenhang bleiben. Ich suche nicht im Internet." }])
  ...
  async function sendMessage(event?: FormEvent) {
    ...
    setLoading(true)
    try {
      const memoryMessages = getMemoryMessages(nextMessages)
      const response = await fetch("/api/arbeitgeber/ai-assistant", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: memoryMessages.map(({ role, content }) => ({ role, content })) }) })
  ...
  return (
    <>
      ...
      {open && <div className="fixed inset-0 z-[80] pointer-events-none sm:inset-auto sm:bottom-6 sm:left-6"><div className="pointer-events-auto absolute bottom-0 left-0 flex h-[min(720px,calc(100vh-24px))] w-full flex-col overflow-hidden rounded-t-[28px] border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.28)] sm:static sm:h-[min(700px,calc(100vh-48px))] sm:w-[430px] sm:rounded-[28px]">
        <header className="flex items-center gap-3 bg-slate-950 px-5 py-4 text-white"><div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-2xl bg-white"><img src="/jobmatch24-logo.png" alt="jobmatch24" className="h-8 w-8 object-contain" /></div><div className="min-w-0 flex-1"><p className="flex items-center gap-1.5 text-sm font-black"><Sparkles className="h-4 w-4 text-blue-300" /> Matchy</p><p className="mt-0.5 text-xs text-slate-300">Hallo Ich bin Matchy dein persönlicher Assistent</p></div><button type="button" onClick={() => setOpen(false)} aria-label="KI-Assistent schliessen" className="rounded-xl p-2 hover:bg-white/10"><X className="h-5 w-5" /></button></header>
        ...
        {messages.length === 1 && <div className="flex flex-wrap gap-1.5 border-t border-slate-100 bg-white px-3 py-2.5">{suggestions.map((suggestion) => <button key={suggestion} type="button" onClick={() => setInput(suggestion)} className="max-w-full rounded-full border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] leading-4 font-bold text-slate-600 hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700">{suggestion}</button>)}</div>}
