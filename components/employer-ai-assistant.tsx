    ...
    <article className="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 flex items-center gap-1.5 text-sm font-bold text-blue-600">
            <BriefcaseBusiness className="h-3.5 w-3.5" />
            {candidate.profession || "Beruf nicht angegeben"}
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <div className="rounded-xl bg-slate-50 p-2.5">
          <p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Ausbildung</p>
          <p className="mt-1 text-xs font-bold text-slate-700">{candidate.education || "Nicht angegeben"}</p>
        </div>
        <div className="rounded-xl bg-slate-50 p-2.5">
          <p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Erfahrung</p>
          <p className="mt-1 text-xs font-bold text-slate-700">{candidate.years_experience ?? 0} Jahre</p>
        </div>
        <div className="rounded-xl bg-slate-50 p-2.5">
          <p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Wunschpensum</p>
          <p className="mt-1 text-xs font-bold text-slate-700">{candidate.desired_employment_percent ?? 100} Prozent</p>
        </div>
        <div className="rounded-xl bg-slate-50 p-2.5">
          <p className="text-[10px] font-black uppercase tracking-wide text-slate-400">Wunschlohn</p>
          <p className="mt-1 text-xs font-bold text-slate-700">{candidate.desired_salary_min == null ? "Nicht angegeben" : `CHF ${candidate.desired_salary_min.toLocaleString("de-CH")}`}</p>
        </div>
      </div>
      ...
      {skills.length > 0 && (
        <div className="mt-3">
          <p className="mb-1.5 text-[10px] font-black uppercase tracking-wide text-slate-400">Fähigkeiten</p>
          <div className="flex flex-wrap gap-1.5">
            {skills.slice(0, 8).map((skill) => <span key={skill} className="rounded-full bg-blue-50 px-2.5 py-1 text-[11px] font-bold text-blue-700">{skill}</span>)}
          </div>
        </div>
      )}
...
export function EmployerAIAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "Hallo! Ich bin dein persönlicher jobmatch24 KI-Assistent. Ich kann innerhalb von JobMatch24 nach passenden Kandidaten suchen, Profile vergleichen und dir bei Bewerbungen helfen. Ich suche nicht im Internet." },
  ])
        ...
        <div className="fixed inset-0 z-[80] pointer-events-none sm:inset-auto sm:bottom-6 sm:left-6">
          <div className="pointer-events-auto absolute bottom-0 left-0 flex h-[min(720px,calc(100vh-24px))] w-full flex-col overflow-hidden rounded-t-[28px] border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.28)] sm:static sm:h-[min(700px,calc(100vh-48px))] sm:w-[430px] sm:rounded-[28px]">
            <header className="flex items-center gap-3 bg-slate-950 px-5 py-4 text-white">
              <div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-2xl bg-white"><img src="/jobmatch24-logo.png" alt="jobmatch24" className="h-8 w-8 object-contain" /></div>
              <div className="min-w-0 flex-1">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto bg-slate-50 p-4">
              {messages.map((message, index) => (
                <div key={`${message.role}-${index}`} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                  {message.role === "user" ? (
                    <div className="max-w-[88%] rounded-2xl rounded-br-md bg-blue-600 px-4 py-3 text-sm leading-6 text-white">{message.content}</div>
                  ) : message.candidates && message.candidates.length > 0 ? (
                    <div className="w-full max-w-[98%]">
                      <div className="mb-3 rounded-2xl border border-blue-100 bg-white p-3 shadow-sm">
                        <div className="flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-xl bg-blue-50 text-blue-600"><Sparkles className="h-4 w-4" /></div><div><p className="text-sm font-black text-slate-950">JobMatch24 – Kandidaten</p><p className="text-xs font-semibold text-slate-500">{message.candidates.length} passende{message.candidates.length === 1 ? "r" : ""} Kandidat{message.candidates.length === 1 ? "" : "en"} gefunden</p></div></div>
                      </div>
                      <div className="space-y-3">{message.candidates.map((candidate) => <CandidateCard key={candidate.id} candidate={candidate} />)}</div>
                    </div>
                  ) : (
                    <div className="max-w-[88%] rounded-2xl rounded-bl-md border border-slate-200 bg-white px-4 py-3 text-sm leading-6 text-slate-700 shadow-sm whitespace-pre-wrap">{message.content}</div>
                  )}
                </div>
              ))}
