...
import { HeroMatchingCard } from "@/components/home/hero-matching-card"
...
const matchValues: [number, string, string][] = [
  [94, "%", "Gesamt-Match"],
  [100, "%", "Beruf"],
  [95, "%", "Erfahrung"],
  [90, "%", "Skills"],
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f7f9fc] text-slate-950">
      <section className="relative overflow-hidden">
        <div className="stoyan-blob-1 pointer-events-none absolute left-1/2 top-[-250px] h-[650px] w-[1000px] -translate-x-1/2 rounded-full bg-blue-500/10 blur-[130px]" />
        <div className="stoyan-blob-2 pointer-events-none absolute right-[-200px] top-[300px] h-[450px] w-[450px] rounded-full bg-indigo-400/10 blur-[120px]" />
        <div className="relative mx-auto max-w-7xl px-6 pb-24 pt-20 sm:pb-32 sm:pt-28 lg:pt-32">
          <div className="grid items-center gap-16 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="max-w-3xl">
              <div className="inline-flex animate-fade-up items-center gap-2 rounded-full border border-blue-200 bg-white px-4 py-2 text-sm font-bold text-blue-700 shadow-sm">
                <Sparkles className="h-4 w-4" />
                Die moderne Plattform für Arbeit
              </div>
              <h1 className="mt-7 max-w-4xl animate-fade-up text-5xl font-black leading-[1.02] tracking-[-0.055em] text-slate-950 [animation-delay:80ms] sm:text-6xl lg:text-7xl">
                Menschen und
                <span className="block stoyan-gradient-text">Unternehmen.</span>
                Besser verbunden.
              </h1>
              <p className="mt-7 max-w-2xl animate-fade-up text-lg leading-8 text-slate-600 [animation-delay:160ms] sm:text-xl">
              ...
              <div className="mt-8 flex animate-fade-up flex-wrap gap-x-6 gap-y-3 text-sm font-semibold text-slate-500 [animation-delay:320ms]">
                ...
                <ScrollReveal key={step.number} delay={index * 120}>
                  <TiltCard max={7} className="h-full">
                    <div className="h-full rounded-3xl border border-slate-200 bg-white p-7 shadow-sm">
                      ...
                      <h3 className="mt-8 text-xl font-black" style={{ transform: "translateZ(22px)" }}>{step.title}</h3>
                      <p className="mt-3 text-sm leading-7 text-slate-500" style={{ transform: "translateZ(14px)" }}>{step.text}</p>
                    </div>
                  </TiltCard>
                </ScrollReveal>
              )
      ...
      <section className="bg-white">
        <div className="mx-auto max-w-7xl px-6 py-20 sm:py-24">
          <div className="grid gap-6 lg:grid-cols-2">
            <ScrollReveal from="left">
              <div id="arbeitnehmer" className="relative h-full overflow-hidden rounded-[30px] bg-slate-950 p-8 text-white sm:p-10">
                <div className="stoyan-blob-1 absolute right-[-80px] top-[-80px] h-64 w-64 rounded-full bg-blue-600/20 blur-3xl" />
            ...
            <ScrollReveal from="right" delay={120}>
              <div id="arbeitgeber" className="relative h-full overflow-hidden rounded-[30px] border border-slate-200 bg-slate-50 p-8 sm:p-10">
                <div className="stoyan-blob-2 absolute right-[-80px] top-[-80px] h-64 w-64 rounded-full bg-blue-500/10 blur-3xl" />
                <div className="relative">
      ...
      <section id="matching" className="bg-[#f7f9fc]">
        <div className="mx-auto max-w-7xl px-6 py-20 sm:py-24">
          <ScrollReveal className="mx-auto max-w-3xl text-center">
            <div className="stoyan-float mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-600/20"><Sparkles className="h-6 w-6" /></div>
          ...
          <ScrollReveal delay={120}>
            <TiltCard max={5} className="mx-auto mt-12 max-w-4xl">
              <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-[0_20px_60px_rgba(15,23,42,0.07)] sm:p-8">
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" style={{ transform: "translateZ(35px)" }}>
                  {matchValues.map(([value, suffix, label]) => (
                    <div key={label} className="rounded-2xl bg-slate-50 p-5 text-center">
                      <p className="text-3xl font-black text-blue-600"><AnimatedCounter value={value} suffix={suffix} /></p>
                      <p className="mt-1 text-xs font-bold uppercase tracking-wider text-slate-400">{label}</p>
                    </div>
                  ))}
                </div>
                <div className="mt-5 rounded-2xl bg-slate-950 p-5 text-white" style={{ transform: "translateZ(20px)" }}>
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-600"><Check className="h-5 w-5" /></div>
                    <div>
                      <p className="font-black">Warum passt dieser Kandidat?</p>
                      <p className="mt-1 text-sm text-slate-400">Beruf passt · Erfahrung passt · Skills passen · Ort passt</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="mx-auto max-w-7xl px-6 py-20 sm:py-24">
          <ScrollReveal>
            <div className="relative overflow-hidden rounded-[30px] bg-blue-600 px-7 py-12 text-center sm:px-12 sm:py-16">
              <div className="stoyan-blob-1 absolute left-1/2 top-[-180px] h-96 w-96 -translate-x-1/2 rounded-full bg-white/10 blur-3xl" />
              <div className="relative">
                ...
                <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
                  <Link href="/registrieren?role=employee" className="rounded-xl bg-white px-6 py-3.5 font-bold text-blue-700 transition hover:bg-blue-50 hover:-translate-y-0.5">Als Arbeitnehmer starten</Link>
                  <Link href="/registrieren?role=employer" className="rounded-xl border border-white/30 bg-white/10 px-6 py-3.5 font-bold text-white backdrop-blur transition hover:bg-white/20 hover:-translate-y-0.5">Als Arbeitgeber starten</Link>
                </div>
              </div>
            </div>
          </ScrollReveal>
