...
type EmployeeProfile = { id: string; beruf: string | null; education: string | null }
...
export default async function ArbeitgeberAnfragenPage() {
  ...
  const [{ data: employeeRows }, { data: profileRows }] = ids.length
    ? await Promise.all([
        supabase.from("employee_profiles").select("id,beruf,education").in("id", ids),
    ...
    {rows.length === 0 ? <section className="card card-pad mt-8 text-center"><MessageCircle className="mx-auto h-10 w-10 text-[var(--brand)]" /><h2 className="mt-4 text-xl font-black">Noch keine Kontaktanfragen</h2><p className="mt-2 text-sm text-[var(--muted)]">Sobald du einen Arbeitnehmer kontaktierst, erscheint die Anfrage hier.</p><Link href="/arbeitgeber/suche" className="btn-primary mt-6 inline-flex">Kandidaten suchen<ArrowRight className="h-4 w-4" /></Link></section> : <div className="mt-8 space-y-4">{rows.map((r) => {
      ...
      return <section key={r.id} className={`rounded-3xl border bg-white p-6 shadow-sm ${accepted ? "border-emerald-200" : "border-[var(--line)]"}`}>
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-4"><div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-[var(--navy)] text-xl font-black text-white">{name.split(" ").slice(0,2).map((x) => x[0]).join("").toUpperCase() || "A"}</div><div className="min-w-0"><p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--muted-light)]">Arbeitnehmer</p><h2 className="mt-1 truncate text-2xl font-black text-[var(--navy)]">{name}</h2><p className="mt-1 text-sm text-[var(--muted)]">{employee?.beruf || "Beruf nicht angegeben"}{employee?.education ? ` · ${employee.education}` : ""}{profile?.city ? ` · ${profile.city}` : ""}</p></div></div>
