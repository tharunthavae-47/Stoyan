...
type Employee = { id: string; first_name: string | null; last_name: string | null; phone: string | null; city: string | null; profession: string | null; education: string | null }
...
export default async function ArbeitgeberAnfragenPage() {
  ...
  const { data: employeeRows } = ids.length ? await supabase.from("employee_contact_details").select("id,first_name,last_name,phone,city,profession,education").in("id", ids) : { data: [] as Employee[] }
  ...
  return <div className="animate-fade-up mx-auto max-w-5xl">
    ...
    <p className="mt-3 max-w-2xl text-[var(--muted)]">Hier findest du alle laufenden Kontaktanfragen. Angenommene Kontakte kannst du direkt öffnen und den privaten Chat auswählen.</p>

    {rows.length === 0 ? <section className="card card-pad mt-8 text-center"><MessageCircle className="mx-auto h-10 w-10 text-[var(--brand)]" /><h2 className="mt-4 text-xl font-black">Noch keine Kontaktanfragen</h2><p className="mt-2 text-sm text-[var(--muted)]">Sobald du einen Arbeitnehmer kontaktierst, erscheint die Anfrage hier.</p><Link href="/arbeitgeber/suche" className="btn-primary mt-6 inline-flex">Kandidaten suchen<ArrowRight className="h-4 w-4" /></Link></section> : <div className="mt-8 space-y-4">{rows.map((r) => { const e = employees.get(r.employee_id); const name = [e?.first_name, e?.last_name].filter(Boolean).join(" ") || "Arbeitnehmer"; const accepted = r.status === "accepted"; return <section key={r.id} className={`rounded-3xl border bg-white p-6 shadow-sm ${accepted ? "border-emerald-200" : "border-[var(--line)]"}`}>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-4"><div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-[var(--navy)] text-xl font-black text-white">{name.split(" ").slice(0,2).map((x) => x[0]).join("").toUpperCase() || "A"}</div><div className="min-w-0"><p className="text-xs font-black uppercase tracking-[0.16em] text-[var(--muted-light)]">Arbeitnehmer</p><h2 className="mt-1 truncate text-2xl font-black text-[var(--navy)]">{name}</h2><p className="mt-1 text-sm text-[var(--muted)]">{e?.profession || "Beruf nicht angegeben"}{e?.education ? ` · ${e.education}` : ""}{e?.city ? ` · ${e.city}` : ""}</p></div></div>
      ...
      {accepted && <div className="mt-5 rounded-2xl border border-emerald-100 bg-emerald-50 p-4"><p className="text-sm font-bold text-emerald-900">Dieser Arbeitnehmer hat angenommen.</p><p className="mt-1 text-sm text-emerald-800/80">Der private Chat ist freigeschaltet. Telefonnummer und E-Mail sind gemäß Freigabe sichtbar.</p></div>}
    ...
    <div className="mt-6 flex items-center gap-2 text-sm text-[var(--muted)]"><Building2 className="h-4 w-4" /> Arbeitgeber links, Arbeitnehmer rechts im Chat.</div>
  </div>
}
