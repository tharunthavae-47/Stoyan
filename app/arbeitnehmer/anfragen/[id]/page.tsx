...
export default function ContactRequestPage({ params }: Props) {
  ...
  return (
    <div className="animate-fade-up"><div className="mx-auto max-w-5xl">
      ...
      <section className="card card-pad"><p className="text-sm font-black uppercase tracking-widest text-[var(--brand)]">Kontaktanfrage</p><div className="mt-5 flex flex-col gap-5 sm:flex-row sm:items-center"><div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-[var(--navy)] text-3xl font-black text-white">{companyAvatar ? <img src={companyAvatar} alt="Unternehmensprofilbild" className="h-full w-full object-cover" /> : companyName.charAt(0).toUpperCase()}</div><div><h1 className="text-3xl font-black">{companyName}</h1><p className="mt-2 text-[var(--muted)]">{request.company?.industry || "Branche nicht angegeben"}{request.company?.city ? ` · ${request.company.city}` : ""}</p><p className="mt-2 text-sm text-[var(--muted)]">Anfrage vom {new Date(request.created_at).toLocaleDateString("de-CH")}</p></div></div>
        ...
        {request.status === "pending" && <div className="mt-6 flex flex-col gap-3 sm:flex-row"><button type="button" onClick={acceptRequest} disabled={actionLoading} className="rounded-xl btn-primary">{actionLoading ? "Wird gespeichert…" : "Anfrage annehmen"}</button><button type="button" onClick={rejectRequest} disabled={actionLoading} className="rounded-xl border border-slate-200 px-6 py-3 font-bold text-slate-700 transition hover:bg-slate-50">Anfrage ablehnen</button></div>}
      </section>

      {request.status === "accepted" && <section className="mt-7 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-200 bg-[var(--navy)] px-6 py-5 text-white"><div className="flex items-center gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-white/10 text-sm font-black">{companyAvatar ? <img src={companyAvatar} alt="Unternehmensprofilbild" className="h-full w-full object-cover" /> : companyName.charAt(0).toUpperCase()}</div><div><p className="text-xs font-black uppercase tracking-widest text-blue-300">Privater Chat</p><h2 className="mt-1 text-xl font-black">{companyName}</h2></div></div><p className="mt-1 text-sm text-blue-100/80">Arbeitgeber links · Arbeitnehmer rechts</p></div>
